/**
 * PZ Auth+API Backend – Versão 4.1.0 – 2025-09-24 – “Name-Splitting-Logic”
 *
 * - Reintegra o funil de captura de leads com o ConvertKit via Axios.
 * - Adiciona a lógica para separar o nome completo do utilizador em "primeiro nome"
 * e "apelido" para uma personalização de e-mail mais flexível.
 * - Preserva toda a base de código estável da versão 2.2.1.
 */
...
─────────────────────────────────────────────────────────────── */
app.get('/', (req, res) => res.status(200).send('🚀 PZ Auth+API Backend ativo. Use /healthz, /api/version, /api/track, /auth/google.'));
app.get('/healthz', (req, res) => res.status(200).json({ ok:true, uptime:process.uptime(), ts:new Date().toISOString() }));
app.head('/healthz', (req, res) => res.sendStatus(200));
app.get('/api/healthz', (req, res) => res.status(200).json({ ok:true, uptime:process.uptime(), ts:new Date().toISOString() }));
app.head('/api/healthz', (req, res) => res.sendStatus(200));
...
app.get('/api/debug/env-has-sa', (req, res) => {
...
app.options('/auth/google', (req, res) => res.sendStatus(204));
app.options('/api/auth/google', (req, res) => res.sendStatus(204));
...
/* ──────────────────────────────────────────────────────────────
   NOVO ENDPOINT PARA O FUNIL DE LEAD MAGNET
─────────────────────────────────────────────────────────────── */
